...
void print_expand_key(unsigned char * in)
{
	...
	/* let c = 16, since first sub-key is user-supplied key */
	unsigned char c = 16;
	unsigned char i = 1;
	unsigned char a;
	int round = 1;

	/* Since we have 11 rounds (Round 0 up to round 10 for 16-byte key), we need 11 sets of
	16 bytes each, for 128-bit mode (AES). */
	while (c < 176)
	{
		...
		/* Every four blocks (4-bytes) do a complex calculation */
		if (c % 16 == 0)
		{
			schedule_core(t, i);
			i++;
