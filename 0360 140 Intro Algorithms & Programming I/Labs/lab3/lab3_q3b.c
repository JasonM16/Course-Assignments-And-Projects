 ...
 * DESCRIPTION	: This program will read in a four or five digit integer 
				  ...
				  Please enter a 4 or 5 digit integer palindrome: 41444
                  41444 is not a palindrome!

                  Please enter a 4 or 5 digit integer palindrome: 44444
                  44444 is a palindrome!
 */
...
int main (void)
{
	...
	{			
		...
		// see if the user would like to check another number
		printf("\n\nContinue? (y/n): ");
		
		// was having issues while running the program so two calls to getchar 
		// are required for program to run properly
		input = getchar();		
		input = getchar();
		
		// continue the loop if the user wishes to continue
		if (input == 'y' || input == 'Y')
     	{
     		continue;
     	}	     
     	
		// if they enter anything other than 'y', break out of the loop and
		// exit program
     	else
     	 break;
    }
