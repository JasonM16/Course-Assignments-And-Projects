...
void enlargeP3 (FILE *inFile, FILE *outFile)
{
	...
	// iterate through each row and set up new columns
	// to be calculated in the last step before being printed
	// to the ppm file
    for (i = 0; i < rows; i++) 
	{
		// new columns in between original columns
		for(j = 0; j < columns; j++)
		{
			if(originalRow == newHeight-1) break;
				
			temp[newRow][originalColumn].red   = ((temp[originalRow][originalColumn].red   + temp[originalRow+2][originalColumn].red)   /2);
			temp[newRow][originalColumn].green = ((temp[originalRow][originalColumn].green + temp[originalRow+2][originalColumn].green) /2);
			temp[newRow][originalColumn].blue  = ((temp[originalRow][originalColumn].blue  + temp[originalRow+2][originalColumn].blue)  /2);

			if(originalColumn == newWidth-1) break;

			temp[newRow][newColumn].red   = ((temp[originalRow][newColumn].red   + temp[originalRow+2][newColumn].red)   /2);
			temp[newRow][newColumn].green = ((temp[originalRow][newColumn].green + temp[originalRow+2][newColumn].green) /2);
			temp[newRow][newColumn].blue  = ((temp[originalRow][newColumn].blue  + temp[originalRow+2][newColumn].blue)  /2);

			// new columns will be in between original columns so 
			// we have to offset on each pass 
			originalColumn += 2;
			newColumn      += 2;				
		}

		// offset original and new rows
		originalRow+=2;		
		newRow += 2;

		// reset the columns. New columns will be directly
		// to the right and in between original columns.
		originalColumn = 0;
		newColumn = 1;				
	}
	...
	{					
		...
		else if (pid == 0)
		{
			...
			// the parent will print the final row, always.
			if (i == rows *2-2) break;
			fprintf(outFile, "\n");	
